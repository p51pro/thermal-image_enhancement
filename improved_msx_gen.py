...
def process_img(rgb_path, thermal_path, out_msx_path, out_msx_GT_path, out_thermal_path, out_rgb_path):
    ...
    enhanced_img = enhance_img(rgb_roi,thermal_roi)
    msx_out_img = cv2.resize(enhanced_img, (160, 120), interpolation = cv2.INTER_CUBIC)  
    cv2.imwrite(out_msx_path, msx_out_img)
    msx_out_img = cv2.resize(enhanced_img, (640, 480), interpolation = cv2.INTER_CUBIC)  
    cv2.imwrite(out_msx_GT_path, msx_out_img)
...
if __name__ == "__main__":
    dataset_path = "/home/p51pro/UD/Academic/ELEG604/Project/live_demo/SRGAN/input_imgs_raw/"
    output_path = "/home/p51pro/UD/Academic/ELEG604/Project/live_demo/SRGAN/preprocess_output/"
    ...
    out_msx_GT_path = os.path.join(output_path, "msx_GT")
    if not os.path.exists(out_msx_GT_path):
        os.mkdir(out_msx_GT_path)
    ...
    for file_name in os.listdir(thermal_img_path):
        ...
        out_gt_msx = os.path.join(out_msx_GT_path,file_name)
        out_thermal = os.path.join(out_thermal_path,file_name)
        out_rgb = os.path.join(out_rgb_path,file_name)
        process_img(os.path.join(rgb_img_path,file_name), os.path.join(thermal_img_path,file_name), out_msx, out_gt_msx, out_thermal, out_rgb)
