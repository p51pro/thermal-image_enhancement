...
"""
hr_transform = Compose([Resize(size=(480,640)),
    RandomCrop(size=(hr_size, hr_size)),
    RandomFlipHorizontal(),
])
"""
transform  = transforms.Compose([crop(4, 24), augmentation()])

    
"""
nor = Compose([Normalize(mean=(127.5), std=(127.5), data_format='HWC'),
              HWC2CHW()])
lr_transform = Compose([Resize(size=(120, 160)),Resize(size=(lr_size, lr_size))])
...
G.init_build(tlx.nn.Input(shape=(batch_size, 3, lr_size, lr_size)))
D.init_build(tlx.nn.Input(shape=(batch_size, 3, hr_size, hr_size)))
...
def train():
    ...
    train_ds = mydata(GT_path = config.TRAIN.hr_img_path, LR_path = config.TRAIN.lr_img_path, in_memory = True, transform = transform)
    train_ds = DataLoader(train_ds, batch_size = batch_size, shuffle = True, num_workers = 0)
    print("Completed DATA LOADER")
    ...
    for epoch in range(n_epoch_init):
        #print("_______________DETAILS OF train_ds _______________________")
        #print(len(train_ds))
...
def evaluate():
    ...
    load_valid_lr_imgs = tlx.vision.load_images(path=config.VALID.lr_img_path )
    ###========================LOAD WEIGHTS ============================###
    for i in range(100):
        G.load_weights(os.path.join(checkpoint_dir, 'g.npz'), format='npz_dict')
        G.set_eval()
        imid = i  # 24 is pump  0: 企鹅  81: 蝴蝶 53: 鸟  64: 古堡
        valid_hr_img = valid_hr_imgs[imid]
        valid_lr_img = np.asarray(load_valid_lr_imgs[imid])
        ...
        print("[*] save images")
        tlx.vision.save_image(valid_lr_img, file_name='valid_lr_img_{}.png'.format(i), path=save_dir)
        tlx.vision.save_image(valid_hr_img, file_name='valid_hr_img_{}.png'.format(i), path=save_dir)
        tlx.vision.save_image(out, file_name='valid_gen_{}.png'.format(i), path=save_dir)
